...
class SodimacScraper(BaseScraper):
    ...
    async def scrape_url(self, url: str) -> ProductPrice:
        try:
            ...
            price = await self._get_price(self.page, ["[class*='internetPrice'] span", "[id*='internetPrice']"])
            original = await self._get_price(self.page, ["[class*='normalPrice'] span"])
    ...
    async def search_product(self, query: str) -> list[ProductPrice]:
        try:
            await self.page.goto(f"https://www.sodimac.cl/sodimac-cl/search?q={query.replace(' ', '+')}", wait_until="domcontentloaded", timeout=30000)
            await self.page.wait_for_timeout(2000)
