...
class ParisScraper(BaseScraper):
    ...
    async def scrape_url(self, url: str) -> ProductPrice:
        try:
            ...
            price = await self._get_price(self.page, ["[class*='internet-price']", "[class*='best-price']", ".jsx-prices span"])
            original = await self._get_price(self.page, ["[class*='normal-price']"])
    ...
    async def search_product(self, query: str) -> list[ProductPrice]:
        try:
            await self.page.goto(f"https://www.paris.cl/search/?q={query.replace(' ', '+')}", wait_until="domcontentloaded", timeout=30000)
            await self.page.wait_for_timeout(2000)
            links = await self.page.eval_on_selector_all("a[href*='/p/']", "els => [...new Set(els.map(e=>e.href))].slice(0,3)")
