...
class EasyScraper(BaseScraper):
    ...
    async def scrape_url(self, url: str) -> ProductPrice:
        try:
            ...
            name = await self._get_text(self.page, ["h1.product-name", "h1[class*='productName']", "h1"])
            price = await self._get_price(self.page, [".special-price .price", "[class*='internet-price'] .price", ".sales-price .price"])
            original = await self._get_price(self.page, [".old-price .price", ".regular-price .price"])
    ...
    async def search_product(self, query: str) -> list[ProductPrice]:
        try:
            await self.page.goto(f"https://www.easy.cl/catalogsearch/result/?q={query.replace(' ', '+')}", wait_until="domcontentloaded", timeout=30000)
            await self.page.wait_for_timeout(2000)
            links = await self.page.eval_on_selector_all("a.product-item-link", "els => [...new Set(els.map(e=>e.href))].slice(0,3)")
